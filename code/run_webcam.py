...
class TensorFlowMobileNetSSD:
    def __init__(self, det_threshold=0.7, model_path='model/frozen_inference_graph_face.pb'):
    ...
    def detect(self, image):
        ...
        faces = np.array([[int(x1 * w), int(y1 * h), int((x2-x1) * w), int((y2-y1) * h),] for y1, x1, y2, x2 in selected_boxes])
